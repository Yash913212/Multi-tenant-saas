...
async function listUsers(req, res) {
    ...
    try {
        ...
        // CASE B: Regular Admin - View ONLY their own users
         ...
         FROM users 
         WHERE tenant_id = $1 
         ORDER BY created_at DESC`, [targetTenantId]
            );
        }
...
async function createUser(req, res) {
    ...
    try {
        ...
        const limitCheck = await db.query(
            `SELECT max_users, (SELECT COUNT(*) FROM users WHERE tenant_id = $1) as current_count FROM tenants WHERE id = $1`, [targetTenantId]
        );
       ...
       VALUES ($1, $2, $3, $4, $5) RETURNING id, email, full_name, role`, [targetTenantId, email, hash, fullName, role || 'user']
        );
...
async function updateUser(req, res) {
    ...
    try {
        ...
        const result = await db.query(
            `UPDATE users SET full_name = $1, role = $2 WHERE id = $3 RETURNING id, full_name, email, role`, [fullName, role, userId]
        );
