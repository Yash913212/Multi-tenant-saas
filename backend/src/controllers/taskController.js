...
async function createTask(req, res) {
    ...
    try {
       ...
       RETURNING *`, [projectId, tenantId, title, description || null, priority || 'medium', dueDate || null, assignedTo || null]
        );
...
async function updateTaskStatus(req, res) {
    ...
    try {
       ...
       WHERE id = $2 AND tenant_id = $3 RETURNING *`, [status, id, tenantId]
        );
...
async function updateTask(req, res) {
    ...
    try {
        ...
        updated_at = NOW()
       WHERE id = $6 AND tenant_id = $7 RETURNING *`, [title, description, priority, assignedTo, dueDate, id, tenantId]
        );
