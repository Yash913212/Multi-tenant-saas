...
const generateToken = (user) => {
    return jwt.sign({ userId: user.id, role: user.role, tenantId: user.tenant_id },
...
async function registerTenant(req, res) {
    ...
    try {
       ...
       RETURNING id, name, subdomain`, [tenantName, subdomain]
        );
       ...
       RETURNING id, email, full_name, role`, [newTenant.id, adminEmail, hashedPassword, adminFullName]
        );
